...
contract StarNotary is ERC721 {
    ...
    constructor() ERC721("StarToken", "STAR") { }
    ...
    function lookUptokenIdToStarInfo (uint _tokenId) public view returns (string memory) {
        require(bytes(tokenIdToStarInfo[_tokenId].name).length > 0, "The Star should exist");
        return tokenIdToStarInfo[_tokenId].name;
    }

    // Implement Task 1 Exchange Stars function
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        require(bytes(tokenIdToStarInfo[_tokenId1].name).length > 0, "The Star 1 to exchange should exist");
        require(bytes(tokenIdToStarInfo[_tokenId2].name).length > 0, "The Star 2 to exchange should exist");
