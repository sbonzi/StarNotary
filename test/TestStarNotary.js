...
it('lets user2 buy a star and decreases its balance in ether', async() => {
    ...
    let value3 = Number(balanceOfUser2BeforeTransaction);
    assert.isBelow(value2, value3);
  });
...
it('lets 2 users exchange stars', async() => {
    ...
    let starId1 = 7;
    let starId2 = 8;
    await instance.createStar('awesome star of 1', starId1, {from: user1});
    await instance.createStar('awesome star of 2', starId2, {from: user2});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user1 = accounts[1];
    let starId = 10;
    await instance.createStar('Unique Star', starId, {from: user1});
    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(starId);
    // 3. Verify if you Star name is the same
    assert.equal(starName, 'Unique Star');
});